...
import { AddPayRollService } from 'src/app/common/add-pay-roll/add-pay-roll.service';
...
export class EmployeeSalaryHistoryComponent {
  ...
  constructor(private addPayRollService: AddPayRollService,
  ...
  getEmployeeAttendanceRecordByEmployeeId() {
    ...
    this.addPayRollService.getEmployeeAttendanceRecordByEmployeeId(model).subscribe(data => {
      if (data.status) {
