...
export class PayRollComponent {
  ...
  constructor(private payRollService: PayRollService, private componentFactoryResolver: ComponentFactoryResolver,
  ...
  deleteSalaryById(employeeSalarySlipID: any) {
    this.spinnerService.show();
    this.payRollService.DeleteEmployeeSalaries(employeeSalarySlipID).subscribe(data => {
      if (data.status) {
  ...
  setEmployeeSalaryPopup(action: string = '', model: any = {}) {
    ...
    const factory = this.componentFactoryResolver.resolveComponentFactory(AddPayRollComponent);
    const popupInjector = Injector.create({
      providers: [{ provide: 'data', useValue: data }],
      parent: this.injector,
    });
  ...
  paySalary(employeeSalarySlipID: number) {
    this.spinnerService.show();
    this.payRollService.SalaryPay(employeeSalarySlipID).subscribe(data => {
  ...
  showEmployeeSalarySlipList() {
    ...
    this.spinnerService.show();
    this.payRollService.showEmployeeSalarySlipList(model).subscribe(data => {
  ...
  saveChanges(salary: any): void {
    ...
    this.spinnerService.show();
    this.payRollService.UpdateSalaryAllowances(model).subscribe(data => {
