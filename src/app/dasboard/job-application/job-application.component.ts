...
export class JobApplicationComponent {
  ...
  listLookup: any = {};
  isJobPostAccess:boolean=false
  isEditJobPost:boolean=false
  isDeleteJobPost:boolean=false

  constructor(private jobApplicationService: JobApplicationService ,private toastrService: ToastrService, private spinnerService: NgxSpinnerService,
    private userContextService: UserContextService, private postJobService: PostJobService, private componentFactoryResolver: ComponentFactoryResolver,
    ...
    this.isJobPostAccess = userRoles.includes(104);
  ...
  GetSetupLookUpData() {
    ...
    this.jobApplicationService.GetSetupLookUpData(model).subscribe(data => {
      if (data.status) {
